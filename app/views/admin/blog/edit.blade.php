...
@section('title', 'Admin | Editar Artículo')
...
@section('content')
    ...
    <div class="py-4 sm:py-6 px-2 sm:px-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <select id="categoria_id" name="categoria_id"
                        ...
                        @foreach(\App\Models\BlogCategoria::activas()->get() as $categoria)
                            <option value="{{ $categoria->id }}" {{ $blog->categoria_id == $categoria->id ? 'selected' : '' }}>
                                {{ $categoria->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select id="autor_id" name="autor_id"
                        ...
                        @foreach(\App\Models\BlogAutor::activos()->get() as $autor)
                            <option value="{{ $autor->id }}" {{ $blog->autor_id == $autor->id ? 'selected' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="resumen" name="resumen" rows="3"
                        class="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $blog->resumen }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="border rounded p-4 bg-gray-50">
                        <div id="preview-container" class="{{ $blog->imagen ? '' : 'hidden' }} mb-4">
                            <img id="preview-image" src="{{ $blog->imagen }}" alt="Vista previa" class="max-h-48 mx-auto">
                        </div>
                        <div class="flex items-center justify-center">
                            <label for="imagen"
                                class="cursor-pointer bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 inline-block">
                                <i class="fas fa-upload mr-2"></i>
                                {{ $blog->imagen ? 'Cambiar imagen' : 'Seleccionar imagen' }}
                            </label>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="hidden" id="contenido" name="contenido" value="{!! $blog->contenido !!}">
                    ...
                    <div class="border rounded p-4 bg-gray-50">
                        <div class="mb-3">
                            ...
                            <input type="hidden" id="selected-tags" name="tags"
                                value="{{ implode(',', $blog->tags_existentes ?? []) }}">
                        </div>
                        ...
                        <div>
                            ...
                            <div class="max-h-40 overflow-y-auto p-2 border rounded bg-white">
                                <div id="available-tags" class="flex flex-wrap gap-2">
                                    @php
                                        $tagsExistentes = $blog->tags_existentes ?? [];
                                    @endphp
                                    @foreach(\App\Models\BlogTag::all() as $tag)
                                        <span
                                            class="tag-item cursor-pointer px-2 py-1 bg-gray-200 text-gray-800 rounded-full text-sm hover:bg-gray-300 {{ in_array($tag->id, $tagsExistentes) ? 'opacity-50' : '' }}"
                ...
                <div>
                    ...
                    <select id="estado" name="estado"
                        class="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500">
                        <option value="borrador" {{ $blog->estado == 'borrador' ? 'selected' : '' }}>Borrador</option>
                        <option value="publicado" {{ $blog->estado == 'publicado' ? 'selected' : '' }}>Publicado</option>
                        <option value="archivado" {{ $blog->estado == 'archivado' ? 'selected' : '' }}>Archivado</option>
                    </select>
                </div>

                <div class="{{ $blog->estado != 'publicado' ? 'hidden' : '' }}">
                    ...
                    <input type="datetime-local" id="fecha_publicacion" name="fecha_publicacion"
                        class="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                        value="{{ $blog->fecha_publicacion ? date('Y-m-d\TH:i', strtotime($blog->fecha_publicacion)) : '' }}">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="meta_description" name="meta_description" rows="2"
                        class="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500">{{ $blog->meta_description }}</textarea>
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="meta_keywords" name="meta_keywords"
                        class="w-full p-2 border rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="palabra1, palabra2, palabra3..." value="{{ $blog->meta_keywords }}">
                    <p class="text-sm text-gray-500 mt-1">Palabras clave separadas por comas.</p>
                </div>
            </div>

            {{-- Botones de acción --}}
            <div class="flex justify-between gap-2 mt-6 border-t pt-4">
                <div>
        ...
        <div class="mt-6 bg-white rounded-lg shadow-md p-4 sm:p-6">
            <h3 class="text-lg font-semibold mb-4">Estadísticas del artículo</h3>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-gray-50 p-4 rounded-lg border">
                    <p class="text-sm text-gray-500">Fecha de creación</p>
                    <p class="text-lg font-medium">{{ date('d/m/Y H:i', strtotime($blog->created_at)) }}</p>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg border">
                    <p class="text-sm text-gray-500">Última actualización</p>
                    <p class="text-lg font-medium">{{ date('d/m/Y H:i', strtotime($blog->updated_at)) }}</p>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg border">
                    <p class="text-sm text-gray-500">Vistas</p>
                    <p class="text-lg font-medium">{{ $blog->vistas ?? 0 }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Mostrar etiquetas ya seleccionadas
            if (selectedTags.length > 0) {
                // Añadir las etiquetas ya seleccionadas
        ...
        document.getElementById('blogForm').addEventListener('submit', function (e) {
            ...
            formData.append('_method', 'PUT'); // Simular método PUT para el servidor
            ...
            // Enviar solicitud
            fetch('/admin/blog/actualizar/{{ $blog->id }}', {
                method: 'POST',
                headers: {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // 🔥 SOLUCIÓN DEFINITIVA: Cargar contenido HTML correctamente
            function cargarContenidoEnEditor() {
                const contenidoInicial = document.getElementById('contenido').value;

                if (contenidoInicial && contenidoInicial.trim() !== '') {
                    console.log("Cargando contenido:", contenidoInicial.substring(0, 100) + "...");

                    // Limpiar el editor primero
                    window.quill.setText('');

                    // Usar dangerouslyPasteHTML que es el método correcto para HTML
                    window.quill.clipboard.dangerouslyPasteHTML(contenidoInicial);

                    console.log("Contenido cargado en Quill");
                } else {
                    console.log("No hay contenido inicial para cargar");
                }
            }

            // Intentar cargar el contenido después de que Quill esté completamente inicializado
            // Usamos múltiples estrategias para asegurar que funcione

            // Estrategia 1: Usar el evento ready de Quill (si existe)
            if (window.quill.ready) {
                window.quill.ready.then(cargarContenidoEnEditor);
            } else {
                // Estrategia 2: Usar setTimeout para asegurar que Quill esté listo
                setTimeout(cargarContenidoEnEditor, 100);
            }

            // Estrategia 3: Backup - intentar después de un tiempo más largo
            setTimeout(function () {
                const contenidoActual = window.quill.getText().trim();
                if (contenidoActual === '' || contenidoActual === '\n') {
                    console.log("Reintentando cargar contenido...");
        ...
        `;
            ...
            // Mostrar/ocultar fecha de publicación según el estado
            ...
            document.getElementById('btnPreview').addEventListener('click', function () {
                // Actualizar el contenido del editor Quill
                const contenidoHTML = window.quill.root.innerHTML;

                // Crear una nueva ventana con el contenido del artículo
                ...
                <body class="bg-gray-100">
                    <div class="max-w-4xl mx-auto my-8 p-6 bg-white rounded-lg shadow-md">
                        <div class="border-b pb-4 mb-6">
                            ...
                            <p class="text-gray-500 mt-2">Vista previa - ${new Date().toLocaleString()}</p>
                        </div>
