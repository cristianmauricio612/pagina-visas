...
@section('content')
        ...
        <div class="bg-white p-6 rounded-lg shadow-md border max-w-3xl mx-auto">
            <form id="createVisaForm">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="tiempo_validez" id="tiempo_validez" class="p-2 border rounded w-full" placeholder="Ejemplo: 2 dias - meses - años" required>
                </div>

                {{-- Número de Entradas --}}
                <div class="mb-4">
                    <label for="numero_entradas" class="block text-gray-700 font-semibold mb-2">Número de Entradas</label>
                    <input type="text" name="numero_entradas" id="numero_entradas" class="p-2 border rounded w-full" placeholder="Ejemplo: Entrada multiple" required>
                </div>

                {{-- Estancia Máxima --}}
                <div class="mb-4">
                    <label for="estancia_maxima" class="block text-gray-700 font-semibold mb-2">Estancia Máxima</label>
                    <input type="text" name="estancia_maxima" id="estancia_maxima" class="p-2 border rounded w-full" placeholder="Ejemplo: 2 dias - meses - años" required>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" step="0.01" name="precio" id="precio" class="p-2 border rounded w-full" placeholder="00.00" required>
                </div>
    ...
    <script>
        ...
        document.getElementById("createVisaForm").addEventListener("submit", function(event) {
            ...
            .then(result => {
                if (result.status === 201) {
                    console.log("✅ Visa creada:", result.body);
                    alert("✅ Visa creada exitosamente");
                    setTimeout(function () {
                        location.reload();
                    }, 500);
                } else {
                    console.error("❌ Error al crear visa:", result.body);
